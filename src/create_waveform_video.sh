...
function show_usage() {
  ...
  echo "  -i, --input            : Path to the input audio file (required)."
  echo "  -o, --output           : Path for the final output video (optional)."
  echo "  -t, --temp             : Path for a temporary working directory (optional)."
  echo
  echo "Metadata Options:"
  echo "  --show-filename        : Render the input filename in the top left."
  echo "  --show-timecode-ms     : Render timecode with milliseconds in the bottom right."
  echo "  --show-timecode-s      : Render timecode in HH:MM:SS at the bottom center."
}
function cleanup() {
...
while [[ "$#" -gt 0 ]]; do
  ...
  --show-timecode-ms) SHOW_TIMECODE_MS=true ;;
  --show-timecode-s) SHOW_TIMECODE_S=true ;;
  -h | --help)
    show_usage
    exit 0
    ;;
...
if [[ "$SHOW_FILENAME" == true || "$SHOW_TIMECODE_MS" == true || "$SHOW_TIMECODE_S" == true ]]; then
  if [[ ! -f "$FONT_FILE" ]]; then
    log "ERROR: Font file not found at '$FONT_FILE'. Please update the FONT_FILE variable in the script." >&2
    exit 1
  fi
fi
if ! command -v ffmpeg &>/dev/null || ! command -v ffprobe &>/dev/null; then
  log "ERROR: ffmpeg and ffprobe are required." >&2
  exit 1
fi
...
# MODIFIED: Corrected the logic for building the filter chain
drawtext_filters=""
if [[ "$SHOW_FILENAME" == true ]]; then
  FILENAME_TEXT=$(basename "$INPUT_FILE" | sed "s/'/\\\\'/g; s/:/\\\\:/g")
